...
func _ready():
	var f = File.new()
	f.open("../dxffiles/aamaout/170color/shapes.json", File.READ)
	...
	for i in range(len(js)):
		...
		rb.name = j["blockname"]
		rb.loadgeometry(j["outercontour"])
...
func sailpanelmousedown(sailpanel, lmousedownposition):
	if sailpaneldragging == null:
		...
		#sailpaneldragging.mode = RigidBody2D.MODE_STATIC
	...
	if sailpaneldragging != null:
		if mousedown:
			...
			if sailpaneldragging != null:
				...
				sailpaneldragging.mode = RigidBody2D.MODE_RIGID
